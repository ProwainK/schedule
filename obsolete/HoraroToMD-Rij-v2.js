...
function asioxGetJSON() {
  axios.get('https://horaro.org/-/api/v1/events/rtaij/schedules/rtaijw2023')
    .then(function (response) {
      //rij_data = response.data.data.items[0];
      rij_data = response.data.data;
      gamedb_rawdata = fs.readFileSync('gamedb.json');
      gamedb_data = JSON.parse(gamedb_rawdata);
      //console.log(rij_data);
      writeStream = fs.createWriteStream("HoraroToMD-RiJ.txt");
      eventInfo();
...
function myFunction(item, index) {
  ...
  game_console = rij_data.items[index].data[2];
  game_type = rij_data.items[index].data[3];
  game_runner = rij_data.items[index].data[5];
